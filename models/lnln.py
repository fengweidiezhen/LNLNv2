...
class LNLN(nn.Module):
    def __init__(self, args):
        ...
        self.reconstructor = nn.ModuleList([
            Transformer(num_frames=args['model']['reconstructor']['input_length'], 
                        ...
                        mlp_dim=args['model']['reconstructor']['hidden_dim']) for _ in range(2)
        ])
        # to be changed
    ...
    def forward(self, complete_input, incomplete_input):
        ...
        h_1_p = self.proxy_dominate_modality_generator(torch.cat([h_0_p, h_1_a], dim=1))[:, :8]
        ...
        h_hyper = self.dmml[1](h_d_list, h_1_a, h_hyper)
        ...
        if (vision is not None) and (audio is not None) and (language is not None):
            ...
            rec_feat_l = self.reconstructor[1](h_1_l)
            rec_feats = torch.cat([rec_feat_a, rec_feat_l], dim=1)
            ...
            complete_feats = torch.cat([complete_audio_feat, complete_language_feat], dim=1) # as the label of reconstruction
